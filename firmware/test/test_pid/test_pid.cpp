...
void test_pure_proportional(void) {
    float integral = 0.0f, e_prev = 0.0f;
    bool saturated = false;
    tvc::PidResult r = tvc::pid_step(integral, e_prev, saturated, /*x_hat=*/3.0f, /*dt=*/0.01f,
...
void test_output_clamps_and_flags_saturated(void) {
    float integral = 0.0f, e_prev = 0.0f;
    bool saturated = false;
    tvc::PidResult r = tvc::pid_step(integral, e_prev, saturated, /*x_hat=*/50.0f, /*dt=*/0.01f,
...
void test_anti_windup_freezes_integral_while_saturated(void) {
    ...
    tvc::pid_step(integral, e_prev, saturated, /*x_hat=*/10.0f, /*dt=*/1.0f, kp, ki, kd,
                  integral_clamp, max_deflection);
    TEST_ASSERT_TRUE(saturated);
...
void test_integral_clamp_bounds_state_even_when_not_saturated(void) {
    ...
    for (int i = 0; i < 3; ++i) {
        tvc::pid_step(integral, e_prev, saturated, /*x_hat=*/-10.0f, /*dt=*/1.0f,
...
void test_derivative_acts_on_x_hat_change(void) {
    float integral = 0.0f, e_prev = 0.0f;
    bool saturated = false;
    tvc::pid_step(integral, e_prev, saturated, /*x_hat=*/0.0f, /*dt=*/0.1f,
                  /*kp=*/0.0f, /*ki=*/0.0f, /*kd=*/1.0f,
                  /*integral_clamp=*/1000.0f, /*max_deflection=*/1000.0f);
    tvc::PidResult r = tvc::pid_step(integral, e_prev, saturated, /*x_hat=*/-5.0f, /*dt=*/0.1f,
                                      /*kp=*/0.0f, /*ki=*/0.0f, /*kd=*/1.0f,
                                      /*integral_clamp=*/1000.0f, /*max_deflection=*/1000.0f);
    // e goes 0 -> 5, deriv = (5-0)/0.1 = 50, u = kd*deriv = 50.
...
int main(int argc, char **argv) {
    ...
    RUN_TEST(test_derivative_acts_on_x_hat_change);
    return UNITY_END();
}
